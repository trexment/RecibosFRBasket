...
function generarReciboPDF($partidos, $usuario, $retencion, $desde, $hasta)
{
    $pdf = new TCPDF();
    ...
    if (file_exists($logoLeft)) {
        $pdf->Image($logoLeft, 15, 10, 45,25);
    }
    if (file_exists($logoRight)) {
        $pdf->Image($logoRight, 165, 10, 20);
    }
    ...
    $pdf->Cell(0, 10, 'RECIBO ARBITRAL', 0, 1, 'C');
    ...
    // Abreviaciones de categorías
    $abreviaciones = [
        'JUNIOR MASCULINO' => 'JM',
        'JUNIOR FEMENINO' => 'JF',
        'PRIMERA DIVISIÓN NACIONAL MASCULINA' => 'PNM',
        'PRIMERA DIVISIÓN NACIONAL FEMENINA' => 'PNF',
        'CADETE MASCULINO' => 'CM',
        'CADETE FEMENINO' => 'CF',
    ...
    foreach ($partidos as $p) {
        ...
        $cat = preg_replace('/\s+/', ' ', $cat); // limpiar espacios extra

// Generar abreviatura automática (primera letra de cada palabra)
        $catWords = explode(' ', $cat);
        $catAbbr = '';
        foreach ($catWords as $word) {
            if (mb_strlen($word) > 2) { // Evitar abreviar preposiciones o artículos cortos
        ...
        $maxLen = 25; // límite de caracteres por nombre
        ...
        $pdf->Cell(40, 6, number_format($p['importe'], 2, ',', '.') . " €", 0, 1, 'R');
    ...
    $total_liquido = $total - $retencion_importe;

    $pdf->Ln(5);
    $pdf->SetFont('helvetica', 'B', 10);
    $pdf->Cell(135, 6, 'TOTAL BRUTO', 0, 0, 'R');
    $pdf->SetFont('helvetica', '', 10);
    $pdf->Cell(45, 6, number_format($total, 2, ',', '.') . " €", 0, 1, 'R');

    if ($retencion > 0) {
        $pdf->SetFont('helvetica', 'B', 10);
        $pdf->Cell(135, 6, "RETENCIÓN IRPF ($retencion%)", 0, 0, 'R');
        $pdf->SetFont('helvetica', '', 10);
        $pdf->Cell(45, 6, "- " . number_format($retencion_importe, 2, ',', '.') . " €", 0, 1, 'R');
    }

    $pdf->SetFont('helvetica', 'B', 10);
    $pdf->Cell(135, 6, 'TOTAL LÍQUIDO', 0, 0, 'R');
    $pdf->SetFont('helvetica', '', 10);
    $pdf->Cell(45, 6, number_format($total_liquido, 2, ',', '.') . " €", 0, 1, 'R');
    ...
    // === DESCARGA ===
    $fileName = 'Recibo_' . str_replace(' ', '_', $usuario['nombre'] . '_' . $usuario['apellidos']) . '_Partidos.pdf';
